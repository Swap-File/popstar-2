...
void public_led(bool rumble,uint8_t leds){

  static bool last_rumble = false;
  static uint8_t last_leds = 0x01;

  if (last_rumble == rumble && last_leds == leds) // only update on change
    return;
...
last_leds = leds;
  ...
  int idx = l2capFindConnection(saved_ch);
  struct l2cap_connection_t connection = l2capConnectionList[idx];
  ...
  payload[posi++] = 0xA2;  // Output report
  payload[posi++] = 0x11;  // Function:Player LEDs
  payload[posi++] = (uint8_t)((leds << 4) | shaker); // LL:controls the four LEDs
  uint16_t dataLen = posi;
  uint16_t len = make_acl_l2cap_packet(tmpQueueData, saved_ch, pbf, bf, channelID, payload, dataLen);
  sendHciPacket(tmpQueueData, len);
  VERBOSE_PRINT("queued acl_l2cap_single_packet(Set LEDs)");
